...
import { BANK_PROVIDERS, EWALLET_PROVIDERS } from '../data/paymentProviders';
import { PaymentProviderCard } from './PaymentProviderBadge';
...
interface BalancePageProps {
  ...
  onRequestWithdrawal: (data: {
    amount: number;
    method: 'bank_transfer' | 'paypal' | 'wise' | 'usdt' | 'ewallet';
...
export const BalancePage: React.FC<BalancePageProps> = ({
  ...
  const [method, setMethod] = useState<'bank_transfer' | 'paypal' | 'wise' | 'usdt' | 'ewallet'>(
    'bank_transfer'
  );
    ...
    try {
      ...
      if (method === 'bank_transfer') {
        if (!bankName || !accountNumber) {
          setError('Please select your bank and enter your Account / IBAN Number.');
          setLoading(false);
          return;
        }
        ...
        if (!bankName || !accountNumber) {
          setError('Please select your e-wallet provider and enter your registered phone number.');
          setLoading(false);
          return;
        }
        details.bankName = bankName;
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5">
                  {[
                    ...
                    { id: 'ewallet', label: 'E-Wallet', icon: <Smartphone className="w-4 h-4" /> },
              ...
              {method === 'bank_transfer' && (
                <div className="space-y-3">
                      ...
                      {BANK_PROVIDERS.map((p) => (
                        <PaymentProviderCard
                          ...
                          onClick={() => {
                            ...
                            setBankName(p.id === 'other_bank' ? '' : p.name);
                          }}
                        />
                      ))}
                    </div>
                    {selectedProviderId === 'other_bank' && (
                      <input
                        type="text"
                        value={bankName}
                        ...
                        required
                        className="mt-2 w-full px-3 py-2 text-sm rounded-xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-neutral-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:outline-none"
                      />
                    )}
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-neutral-700 dark:text-neutral-300 mb-1">
                        SWIFT / BIC Code <span className="font-normal text-neutral-400">(optional)</span>
                      </label>
                      <input
                        type="text"
                        value={swiftCode}
                        onChange={(e) => setSwiftCode(e.target.value)}
                        placeholder="e.g. HSBCUKLLXXX"
                        className="w-full px-3 py-2 text-sm rounded-xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-neutral-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:outline-none"
                      />
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      {EWALLET_PROVIDERS.map((p) => (
                        <PaymentProviderCard
                          ...
                          onClick={() => {
                            setSelectedProviderId(p.id);
                            setBankName(p.id === 'other_ewallet' ? '' : p.name);
                          }}
                        />
                      ))}
                    </div>
                    {selectedProviderId === 'other_ewallet' && (
                      <input
                        type="text"
                        value={bankName}
                        onChange={(e) => setBankName(e.target.value)}
